...
const requestedProfile = getProfileName() || (packageJson.installationProfile ? '' : inferPackagedProfileName());
const _profileName = resolveInstallationProfile(packageJson.installationProfile, sanitizeProfileName(requestedProfile));
...
const APP_USER_MODEL_ID = packageJson.installationProfile
  ? installationIdentity(packageJson.installationProfile).appId
  : (_profileName ? `com.aephia.lm-market-bot-${_profileName.toLowerCase()}` : 'com.aephia.lm-market-bot');
...
async function downloadUpdateAndRestart() {
  ...
  try {
    ...
    emitUpdateProgress('stopping-bot', `Stopping ${_profileName} safely...`);
    ...
    emitUpdateProgress('restarting', `Updating ${_profileName}; other installations are unaffected.`);
...
handleTrusted('updates:download-and-restart', async () => {
  if (updateRequestInFlight) throw new Error('An update is already in progress.');
  updateRequestInFlight = true;
  try {
    const result = await downloadUpdateAndRestart();
    if (!result.updated) updateRequestInFlight = false;
    return result;
  } catch (error) {
...
// The profile userData path scopes Electron's lock across copies of the same profile.
const hasProfileLock = app.requestSingleInstanceLock();
if (!hasProfileLock) app.quit();
app.on('second-instance', () => {
  if (mainWindow) { if (mainWindow.isMinimized()) mainWindow.restore(); mainWindow.focus(); }
});

app.whenReady().then(async () => {
  if (!hasProfileLock) return;
  if (app.isPackaged) {
